import React from 'react';
import { Lightbulb, Rocket, Sparkles, Award } from 'lucide-react';

export const PolygnanEthosBanner: React.FC = () => {
  return (
    <div className="py-6 space-y-8 relative overflow-hidden">
      {/* Glow Orbs */}
      <div className="pointer-events-none absolute -top-20 -left-20 w-80 h-80 bg-[#C4F62E]/5 rounded-full blur-[100px]" />
      <div className="pointer-events-none absolute -bottom-20 -right-20 w-80 h-80 bg-[#E8B923]/5 rounded-full blur-[100px]" />

      <div className="max-w-4xl mx-auto space-y-8 relative z-10">
        {/* Badge */}
        <div className="text-center space-y-3">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#C4F62E]/10 border border-[#C4F62E]/20 text-xs font-mono-stats text-[#C4F62E] font-bold uppercase tracking-wider">
            <Sparkles className="w-4 h-4 text-[#C4F62E]" />
            Driven by Polygnan · The Rancho Builder Mindset
          </div>

          <h3 className="font-display font-extrabold text-2xl sm:text-4xl text-white tracking-tight leading-tight">
            "You Learn By <span className="text-[#C4F62E]">Doing</span>"
          </h3>

          <p className="text-xs sm:text-base text-[#8A8A85] max-w-2xl mx-auto font-sans leading-relaxed">
            At Polygnan, we want India's youth to become the kind of builders who don't just prepare for the future of work — but help shape it.
            Earn however you want in under 30 days.
          </p>
        </div>

        {/* 3 Pillars Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-[#121212]/80 border border-[#202020] rounded-2xl p-6 space-y-3 hover:border-[#C4F62E]/40 transition-all backdrop-blur-sm">
            <div className="w-10 h-10 rounded-xl bg-[#0A0A0A] border border-[#262626] flex items-center justify-center text-[#C4F62E]">
              <Lightbulb className="w-5 h-5" />
            </div>
            <h4 className="font-display font-extrabold text-base text-white">Rancho Mindset</h4>
            <p className="text-xs text-[#8A8A85] font-sans leading-relaxed">
              Curiosity over credentials. Execution over theory. Solve real problems and create tangible value from day one.
            </p>
          </div>

          <div className="bg-[#121212]/80 border border-[#202020] rounded-2xl p-6 space-y-3 hover:border-[#C4F62E]/40 transition-all backdrop-blur-sm">
            <div className="w-10 h-10 rounded-xl bg-[#0A0A0A] border border-[#262626] flex items-center justify-center text-[#C4F62E]">
              <Rocket className="w-5 h-5" />
            </div>
            <h4 className="font-display font-extrabold text-base text-white">30-Day Income Sprint</h4>
            <p className="text-xs text-[#8A8A85] font-sans leading-relaxed">
              Challenge yourself to generate your first income stream in under 30 days using skills you love.
            </p>
          </div>

          <div className="bg-[#121212]/80 border border-[#202020] rounded-2xl p-6 space-y-3 hover:border-[#C4F62E]/40 transition-all backdrop-blur-sm">
            <div className="w-10 h-10 rounded-xl bg-[#0A0A0A] border border-[#262626] flex items-center justify-center text-[#E8B923]">
              <Award className="w-5 h-5" />
            </div>
            <h4 className="font-display font-extrabold text-base text-white">India Earning Movement</h4>
            <p className="text-xs text-[#8A8A85] font-sans leading-relaxed">
              Lead the movement on your campus as a verified Scout. Unlock stipends, merch, and founding team roles.
            </p>
          </div>
        </div>
      </div>
    </div>
  );
};
